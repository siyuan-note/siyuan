...
* `shell.openExternal()` with unvalidated URLs: In the Electron main process, `shell.openExternal()` is called without a URL-scheme allowlist at `windowNavigate` and `setWindowOpenHandler` (`app/electron/main.js`). Handing off arbitrary URLs to the OS shell is expected behavior because opening external links in the system browser is a common user need; whether the OS-level URI-handler chain is exploitable depends on the user's OS and installed software, outside SiYuan's security boundary
