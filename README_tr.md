...
- [🏗�?Mimari ve Ekosistem](#-mimari-ve-ekosistem)
...
SiYuan’�?bir sunucuda çalıştırmanın en kolay yolu Docker üzerinden dağıtmaktır.
...
SiYuan’�?Docker Compose ile çalıştıran kullanıcılar için, `PUID` ve `PGID` ortam değişkenleri kullanılarak kullanıcı ve grup kimlikleri özelleştirilebilir. Aşağıda örnek bir Docker Compose yapılandırması bulunmaktadır:
...
services:
  main:
    ...
    environment:
      ...
      - SIYUAN_LANG=en       # Arayüz dili (bu belgeyle uyumlu)
```
