...
> 配套文档:[`WORKSPACE.zh-CN.md`](./WORKSPACE.zh-CN.md) 讲工作区在磁盘上的整体布局(笔记本、父子文档、附件的组织方式);本文档专注 `.sy` 文件**内部**的 JSON 结构。
...
> 这些规则由 Lute 的 `CanContain`(`ast/node.go:988`)强制校验,违反会导致解析/渲染异常,AI 生成时必须遵守。
