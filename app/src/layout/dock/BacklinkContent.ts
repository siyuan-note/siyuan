...
export class BacklinkContent extends Model {
    ...
    constructor(options: {
        ...
        onlyBacklinks?: boolean,
        surface?: string,
    }) {
        super({app: options.app});

        if (options.tab && options.type !== "bottom") {
            this.connect({
                id: options.tab.id,
                type: "backlink",
        ...
        this.onlyBacklinks = !!options.onlyBacklinks;
        this.surface = options.surface || options.type;
        ...
        this.element.classList.toggle("sy__backlink--only-backlinks", this.onlyBacklinks);
...
<div class="backlinkMList fn__flex-1"></div>`;
    ...
    private toggleItem(liElement: HTMLElement, isMention: boolean, persist = true) {
        ...
        if (svgElement.classList.contains("b3-list-item__arrow--open")) {
            ...
            if (editor && (this.type === "bottom" || isMobile())) {
        ...
        fetchPost(isMention ? "/api/ref/getBackmentionDoc" : "/api/ref/getBacklinkDoc", param, (response) => {
            ...
            if (!response.data.unchanged) {
                ...
                if (record.editor) {
                    ...
                    /// #if MOBILE
                    registerMobileSecondaryEditor(editor, () => this.switchBlock("", "", ""));
                    /// #endif
    ...
    private searchBacklinks(init = false, refreshAllContexts = false) {
        ...
        if (!notebookId && this.rootId) {
            getAllEditor().some(item => {
                if (item.protyle.block.rootID === this.rootId) {
                    notebookId = item.protyle.notebookId;
                    return true;
                }
            });
        }
        ...
        fetchPost("/api/ref/getBacklink2", param, response => {
            ...
            if (this.editors.some(editor => editor.protyle.element.contains(document.activeElement))) {
                this.markDirty();
                return;
            }
            if (!response.data) {
