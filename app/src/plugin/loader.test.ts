...
test("saved UI layout takes priority over cached placement and keeps new metadata", () => {
    const {load, buttons, storage} = setup({
        ...createConfig(), position: "RightTop", icon: "iconInfo", title: "Cached dock",
    }, {
    ...
    assert.equal(buttons[0].tab.show, false);
    assert.deepEqual(buttons[0].tab.size, {width: 500, height: 300});
