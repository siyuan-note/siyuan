const MOBILE_HINT_USABLE_HEIGHT = 128;
...
export const getMobileHintPosition = (anchorTop: number, anchorBottom: number, hintHeight: number,
    ...
    const usableHeight = Math.min(MOBILE_HINT_USABLE_HEIGHT, Math.max(0, heightLimit));
    const below = spaceAbove < usableHeight && spaceBelow > spaceAbove;
