...
export const getFiltersHTML = (data: IAV) => {
    ...
    const genAndOrSelect = (groupPath: string, combination: string) =>
        `<select class="b3-select av__filter-select" data-type="toggleCombination" data-path="${groupPath}" style="width:${andOrControlWidth}px;"><option value="and" ${combination === "and" ? "selected" : ""}>${window.siyuan.languages.filterCombinationAnd}</option><option value="or" ${combination === "or" ? "selected" : ""}>${window.siyuan.languages.filterCombinationOr}</option></select>`;
...
<option value="0"${showToday ? " selected" : ""}>${window.siyuan.languages.current}</option>
        ...
        const relCount = `<input type="number" min="1" step="1" value="${relativeDate?.count || 1}" class="b3-text-field b3-text-field--text av__filter-input" data-type="relCount${suffix}" data-path="${path}" style="width:50px;${(!relativeDate || showToday) ? "display:none;" : ""}">`;
...
const genInlineSelectHTML = (filter: IAVFilter, colData: IAVColumn, path: string, valueType: TAVCol): { trigger: string, dropdown: string } => {
    ...
    const trigger = `<span data-type="selectTrigger" data-path="${path}" class="av__select-trigger">${triggerContent}</span>`;
...
<span class="fn__ellipsis" style="min-width:0;">${escapeHtml(option.name)}</span>
    ...
    const dropdown = `<div data-type="selectDropdown" data-path="${path}" data-single="${isSingle ? "true" : "false"}" class="av__select-dropdown" style="display:none;">
${searchInput}<div data-type="selectOptions" data-path="${path}" class="av__select-options">${chips}</div>
</div>`;
