...
describe("classifyTextDirection", () => {
    ...
    it("initializes automatic direction before loading a new document", () => {
        const source = readFileSync(resolve(process.cwd(), "src/protyle/util/onGet.ts"), "utf8");
        const syncIndex = source.indexOf("syncAutoDirectionRuntime(options.protyle);");
        const firstSetHTMLIndex = source.indexOf("setHTML({");
        assert.ok(syncIndex >= 0);
        assert.ok(firstSetHTMLIndex >= 0);
        assert.ok(syncIndex < firstSetHTMLIndex);
    });
});
