...
export const getListDocDisplayName = (item: IFile) => {
    if (item.titleEmpty) {
        return window.siyuan.languages["_kernel"][16];
    }
    return getDisplayName(item.name, true, true);
};
...
<span class="counter counter--right b3-tooltips b3-tooltips__w" aria-label="${window.siyuan.languages.flashcardCard}">${item.flashcardCount}</span>`;
    ...
    <span class="b3-list-item__text ariaLabel" data-position="parentE" aria-label="${getListDocDisplayName(item)} <small class='ft__on-surface'>${item.hSize}</small>${item.bookmark ? "<br>" + window.siyuan.languages.bookmark + " " + item.bookmark : ""}${item.name1 ? "<br>" + window.siyuan.languages.name + " " + item.name1 : ""}${item.alias ? "<br>" + window.siyuan.languages.alias + " " + item.alias : ""}${item.memo ? "<br>" + window.siyuan.languages.memo + " " + item.memo : ""}${item.subFileCount !== 0 ? window.siyuan.languages.includeSubFile.replace("x", item.subFileCount) : ""}<br>${window.siyuan.languages.modifiedAt} ${item.hMtime}<br>${window.siyuan.languages.createdAt} ${item.hCtime}">${getListDocDisplayName(item)}</span>
    ${countHTML}
</li>`;
        });
        if (fileHTML === "") {
