...
This document defines type declarations, compatibility requirements, generated artifacts, and verification for kernel HTTP APIs. Transport types and endpoint definitions are maintained in the module files under `kernel/apicontract/`; `contracts.go` collects the endpoint registry. The flashcard branch retains its existing `/api/flashcard/` entries in `kernel/apicontract/legacy_routes.json`; `/api/riff/` compatibility endpoints use typed contracts. New endpoints must define type contracts and must not be added to the legacy list.
...
3. Remove an existing entry from `legacy_routes.json` when binding its endpoint to a contract; when deleting an endpoint, remove its route registration and corresponding contract or legacy entry
...
The route inventory covers method/path registrations in `kernel/api/router.go`, with `ANY` registrations expanded to concrete HTTP methods in generated metadata. Use the generator and route coverage checks for current counts and the existing flashcard entries in the legacy list. Static resources, the main application WebSocket, and other transport services registered by `kernel/server/serve.go` are outside this API route inventory.
...
The quick flashcard creation contract retains struct JSON binding, the `invalid flashcard request: ` error prefix, and administrator/read-only admission. Pure whitespace paragraphs are skipped when creating quick sources; active sources remain eligible for toggle cancellation. An entirely skipped selection succeeds with empty `sourceIDs` and `cardIDs` arrays and `action: "created"`. `TestQuickFlashcardSourcesContract`, `TestEmptyQuickFlashcardParagraph`, and `TestQuickSourceEmptyBlocksPreserveCancellationAndState` cover the request/response protocol, semantic content checks, and cancellation with preserved review state.
