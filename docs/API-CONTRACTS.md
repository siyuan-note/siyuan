...
On Linux, set `TMPDIR` to an existing directory under your home directory before running the full kernel suite. Import and static-file fixtures represent ordinary user data, while `/tmp` outside the workspace is intentionally rejected by the sensitive-path guard. CI uses `${{ runner.temp }}` for this purpose.
