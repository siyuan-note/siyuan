...
本文规定内核 HTTP 接口的类型定义、兼容要求、生成产物和验证流程。传输类型与端点定义位于 `kernel/apicontract/` 下的各模块文件，`contracts.go` 汇总端点注册表。闪卡分支在 `kernel/apicontract/legacy_routes.json` 中保留已有的 `/api/flashcard/` 记录，`/api/riff/` 兼容接口使用类型契约。新增接口必须定义类型契约，不得加入该清单。

## 接口维护流程

1. 在契约包中定义或更新传输类型与端点，明确请求体、错误码、空值、默认值和历史输入兼容规则
2. 用 `contractHandler` 绑定业务入口，保持路由中间件顺序、授权和租约范围
3. 将存量接口绑定到契约时，从 `legacy_routes.json` 移除对应记录；删除接口时同步移除路由注册及对应的契约定义或存量记录
