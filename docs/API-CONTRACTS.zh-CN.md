...
标题事务查询使用 `BlockTransaction` 返回有类型的操作，保留空数组、空值和撤销操作载荷。`BlockOperationResult` 明确声明文本、块 ID 数组和空值的有限联合类型。业务模型中的多态字段转换时拒绝不支持的类型，属性视图操作需要单独声明其载荷契约。编辑器操作类型同时接受非属性视图操作返回的空列类型字段。
