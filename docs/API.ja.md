...
`POST /api/filetree/duplicateDocTree` には管理者権限が必要です。カーネルが読み取り専用の場合は実行できません。

```json
{"id":"20260919010000-source1"}
```

`id` はドキュメント ID を指定します。ノートブックドキュメントは対象外です。指定したドキュメントとすべての子ドキュメントを、同じノートブックの元の親階層に複製します。すべてのブロックに新しい ID を割り当て、複製を示す接尾辞は最上位のタイトルにのみ追加します。子ドキュメントのタイトル、階層、ソート方式、カスタム順序を保持します。カスタム順序では、複製した最上位ドキュメントを元の直後に配置します。

複製対象内のブロック参照とブロックリンクは、タブのタイトルやリッチテキスト表セルも含め、複製先のブロックを参照します。クエリ埋め込み内の明示的なブロック ID も置き換えます。ほかの参照とアセットは元の参照先を保持します。データベースブロックは元のデータベースと行の関連付けを共有するミラーのままなので、関連付けられた行を編集すると元のブロックも変更される場合があります。複製したブロックはデータベースの項目やフラッシュカードへの所属を引き継ぎません。

成功時は複製した最上位ドキュメントを返します。
...
暗号化ノートブックは事前にロックを解除する必要があり、応答のシリアライズ完了までノートブックのリースを保持します。読み取り不能、破損、未対応の形式はエラーとして扱い、元のファイルを保持します。書き込み失敗を検出した場合は新規ドキュメントを削除して順序を復元し、削除に失敗した場合もエラーを返します。この復元処理はプロセスのクラッシュを対象としません。`/api/filetree/duplicateDoc` は引き続き単一ドキュメントのみを複製します。
...
外側の `LIMIT` を明示しない場合、返される結果は既定で `search.limit` 行（設定の検索結果件数）までです。ページ分割には `LIMIT` と `OFFSET` を明示し、`ORDER BY hpath, id` のように順序が安定して一意に決まる並べ替えを使用してください。外側の `LIMIT` を明示すると既定の制限を上書きでき、`search.limit` より大きい値も指定できます。
...
成功時の `data` は配列のままです。`limit` は今回のクエリに適用されたサーバーの既定の上限を表し、SQL に外側の `LIMIT` が明示されている場合は `0` になります。明示された句の値を表すものではありません。`truncated` は、サーバーの既定の制限によって少なくとも1行が返されなかった場合にのみ `true` になります。結果が上限と同じ件数でも、省略された行がなければ `false` です。上記の例では `LIMIT 7` が明示されているため、`limit` は `0`、`truncated` は `false` です。エラー応答にはこの2つのフィールドは含まれません。
