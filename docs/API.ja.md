...
`POST /api/ai/testDecisionModel` には認証と管理者権限が必要です。リクエスト本文は不要です。保存済みの `ai.decision` 設定を使い、固定サンプルで TypeSafe System One API をテストします。ノートの内容は読み取りません。判定モデルが無効でもテストできますが、AI 全体が無効の場合は拒否されます。プロバイダーの料金が発生する場合があります。

有効な回答を受信すると、標準の成功レスポンスに `data: {"matched": true}` が含まれます。設定不足、通信失敗、無効なレスポンスの場合は、`code: 0` と `data: {"matched": false, "msg": "..."}` を返します。接続とレスポンス形式を確認するもので、モデルの精度を検証するものではありません。設定したタイムアウトが適用され、自動で再試行されません。
