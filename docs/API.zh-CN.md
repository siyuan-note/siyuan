...
`POST /api/filetree/duplicateDocTree` 需要管理员权限，内核不能处于只读模式。

```json
{"id":"20260919010000-source1"}
```

`id` 必须为文档 ID，不能为笔记本文档 ID。接口将文档及全部子文档复制到同一笔记本的原父目录。所有块使用新 ID，仅根文档标题增加副本后缀；子文档标题、层级、文档排序方式和自定义子文档顺序保持不变。自定义排序下，副本根文档紧接原文档。

副本内部的块引用和块链接指向副本中的块，包括标签页标题和富文本表格单元格。查询嵌入中的显式块 ID 同样重映射，其他引用和资源文件保留原目标。数据库块仍为镜像，共享原数据库及行绑定，编辑绑定行可能修改源块。副本中的块不继承数据库条目绑定或闪卡归属。
...
加密笔记本须先解锁，请求持有笔记本租约直到响应序列化完成。源文档不可读、损坏或格式不受支持时返回错误并保留源文件。检测到写入失败时清理新建文档并恢复排序，清理失败也会返回错误；此补偿不覆盖进程崩溃。`/api/filetree/duplicateDoc` 仍仅复制单篇文档。
...
* 参数
  ...
  ```
    ...
    * `id`：声明子文档排序方式的普通文档 ID，不接受笔记本根文档 ID
    * `sortMode`：`0` 到 `14` 的整数；传入 `null` 会清除该文档的显式设置，并依次继承最近父文档、笔记本或全局文档树的排序规则
...
* 返回值
  ...
  ```
    ...
    * `sortMode` 为显式设置值（继承时为 `null`），`effectiveSortMode` 为解析继承后的实际生效值
