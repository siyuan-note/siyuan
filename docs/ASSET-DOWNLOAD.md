...
同步合并采用可恢复的应用日志，记录目标资源清单、变更前版本和云端基线。应用前检查本地文件是否仍符合预期，保留下载期间的本地额外修改。中断后可重试，只有文件及引用更新完成后才清除日志。来源切换与资源物化串行化，仓库内部操作共享原有互斥锁，并重新读取已认证的设备状态，避免旧实例覆盖新状态。
...
导出在持有笔记本读锁和创建产物之前预取引用资源，范围包含子文档、关联文档、查询嵌入、题头图、数据库及关联数据库中的资源。目录资源根据逻辑清单补齐，不能只依靠磁盘遍历。下载或复制失败向上传播，不将缺少资源的产物报告为成功。
