...
支持从 `conf/hpath-refresh.json` 迁移版本为 `1` 的记录。两处记录都存在时按笔记本与文档 ID 合并，同一任务优先采用新位置的记录；合并结果持久化成功后才删除旧文件。迁移中断可重试，读取失败、格式损坏或未知版本均保留原文件并报错。
