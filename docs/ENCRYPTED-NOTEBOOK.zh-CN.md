...
`notebook-crypto-backup.json` 是恢复材料而非秘密，但必须处理完整性和新鲜度。文件必须带格式版本、备份 ID、创建时间、内容摘要和基于 KEK 的 HMAC。当前格式强制要求 HMAC：主密码派生 KEK 后，HMAC 缺失或不匹配均拒绝恢复，不得作为兼容路径放行。恢复不得静默覆盖已启用配置。格式无效或与已有加密笔记本不匹配的备份进入错误状态，而不是生成新的 MasterSalt。
...
| 改主密码和 KEK 重新包络 | WrappedDEK 与备份原子更新；更新中断后保持待完成状态，直到新密码认证备份；旧密码泄露风险提示正确；迁移前后数据与历史均可验证读取 |
