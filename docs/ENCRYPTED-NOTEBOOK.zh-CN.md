...
- 父目录不属于 AAD，应用仍按现有目录结构构建文档层级；加载时校验文件基名与解密后根块 ID 一致，并拒绝同一 box 内的重复对象 ID、非法路径和循环层级，但不提供父目录关系的密码学完整性
...
**同一加密 box 内移动**：`.sy` 密文 AAD 只绑定稳定文件基名，不绑定父目录。文件基名不变的移动按普通笔记本的同箱移动语义原样 Rename 密文，不需要重新封装根文档或子文档；只有文件基名本身变化时才使用旧基名验证解密并按新基名重新封装。移动完成后更新索引，失败处理与普通笔记本一致，不额外引入加密层级元数据。
...
| 改密 | 设置 → 访问授权 → "修改主密码" |
| 移动文档 | 加密 box 内部仅改变父目录时原样移动密文并更新索引；文件基名变化时重新封装；普通 box 之间正常移动；跨加密边界（普通↔加密）被拒绝并提示 |
| 文档转标题 | 跨加密边界被拒绝并提示 |
...
- 父目录不进入 AAD，因此能够写工作空间密文的攻击者可以在同一 box 内移动一个文件基名不变的有效密文，应用会把新父目录视为当前文档层级；内容、box、对象类型和稳定对象 ID 仍受认证，但父目录层级本身不提供密码学完整性。
...
| 同一加密 box 内移动文档，随后清空缓存并重启 | 文件基名不变时密文保持不变并可按稳定对象 ID 认证解密；新父目录按普通笔记本语义生效，旧目录位置不存在，失败处理与普通笔记本一致 |
