...
`notebook-crypto-backup.json` 是恢复材料而非秘密，但必须处理完整性和新鲜度。文件必须带格式版本、备份 ID、创建时间、单调递增世代和内容摘要。恢复不得静默覆盖已启用配置；本地未启用且存在多个候选备份时，必须展示来源与世代供用户选择。格式无效、世代回退或与已有加密笔记本不匹配的备份进入错误状态，而不是生成新的 MasterSalt。
...
| 密文篡改、路径替换、nonce 重复、备份损坏/回滚、多端冲突 | 认证或格式校验失败并安全拒绝；不回退普通路径、不生成新密钥材料、不静默覆盖配置 |
