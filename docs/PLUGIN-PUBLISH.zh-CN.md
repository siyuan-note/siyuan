...
资源采用完整相对文件名，或以 `/` 结尾的目录路径。例如，`"fonts/"` 发布该目录及其子目录中的所有文件，`"fonts"` 仍表示精确文件名。分隔符为 `/`，不支持通配符、绝对路径、上级目录跳转、百分号编码及链接。不得声明 `plugin.json` 或 `kernel.js`。最多声明 4096 项资源及 128 个数据字段。加载额外脚本、图片、字体或 HTML 的插件需声明相应文件或目录，只使用标准入口的插件无需补充资源声明。目录声明也包含后续新增文件，因此请将私有文件放在公开目录之外。
