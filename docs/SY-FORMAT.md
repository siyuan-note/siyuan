...
The internal Markdown syntax uses `:::tabs`, `:::tab <inline title>` and a separate `:::` for each closing item/container. A closing fence's following IAL belongs to that node. Fences inside code blocks are literal. Standard Markdown exports title paragraphs followed by every item's body; HTML can enhance the full content into interactive tabs, while print, PDF and Word show all items. See [Tabs design](TABS.md) for the full contract.
