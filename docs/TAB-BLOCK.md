...
Clicking a state icon does not select the tab or start renaming: incomplete becomes `X`, while any other state becomes a space. Right-clicking the icon or choosing Task status in the title menu offers To do, In progress, Completed, Canceled and Custom task status. The original Custom task status entry remains available. In progress uses `/`; Canceled uses `-` and dims and strikes through the navigation title. In-progress titles retain their normal appearance. The icon has no separate Task List tooltip.

Task list items offer the same preset and custom states by right-clicking the task icon or using the block menu - List block - Task status. The block menu - List block - Custom task status still opens single-character input directly. Task icons display the original custom character. In-progress text retains its normal appearance, canceled text is dimmed and struck through, and completed text keeps its existing style. Display follows the original marker; the stored compatibility rule that non-space markers are checked remains unchanged, as does the existing Markdown export normalization. Setting and toggling states support undo and redo. Copying, cutting or dragging an individual tab item materializes its inherited incomplete state in the destination so it remains a task outside its original group; undoing a move restores the original inherited attribute state.
