...
Verify empty-container normalization, round trips for string titles and separate title paragraphs, selection repair, ID remapping during copying and import, and Markdown fences at different nesting depths. Editing checks cover title formatting, IME composition, task state, list and super-block conversion, undo, deletion of the last item, and moves across groups. UI checks cover top and left layouts, narrow-container fallback, keyboard navigation, temporary read-only selection, and updates to hidden items. Export checks cover structured and flattened Markdown, printing every item, title-resource scanning, and rejection of unsupported document versions.
