...
- `TabItemTitle` 保存行级 Markdown，标题能力沿用提示块。标题允许重复和清空，空标题的占位提示不写入正文。
...
页签项的 IAL 紧接标题标记下一行，中间不留空行；正文在其后的空行之后开始。组结束围栏之后的 IAL 属于整组容器，正文块的 IAL 仍紧随对应正文块。该位置约定区分页签项与首个空段落的属性，分别保存各节点的 ID 和属性。
...
列表转页签时，从每个直属列表项首个正文块提取标题；仅当该块为段落时使用其可见文字，将换行和连续空白合并为空格，取前 12 个 rune，超出时追加不计入长度的 `...`。标题使用独立的纯文本摘要，原段落及其行级格式完整保留在正文中；没有可用文字时保留空标题。标题只在转换时生成，之后独立编辑。转换后默认选择第一页。

页签转列表时，将非空页签标题作为列表项首段落，原正文接在后面；普通文字标题与原正文首段落的可见文字相同时，不重复插入。富格式标题还需行级内容完全一致才省略，避免丢失标题中的引用等信息。新插入的标题段落获得新 ID，保留标题行级格式。截断标题可能与原正文部分重复，以保留用户修改后的标题。有序列表从 1 开始，任务列表全部未完成；不保留原列表编号、勾选状态或页签布局、选中状态用于后续反向转换。
...
页签标题需要显式接入全文索引、引用与资源扫描、动态锚文本和历史差异。标题的临时行级树关联页签项 ID，但不永久加入存储的正文子树。块命名优先于标题作为引用锚文本。页签内标题与提示块内标题一样，不参与文档标题编号；折叠不能越过页签项边界。

HTML 保留交互，初始化前或脚本失败时显示全部内容。打印、PDF 和 Word 递归展开全部页签并完成内容渲染。标准 Markdown 按顺序输出页签标题段落和正文，保留标题行级格式及正文原有标题级别；内部 Markdown 保留完整结构和属性。
