...
围栏独占一行，标题是 `:::tab` 后的行级内容。结束标记关闭当前最近的页签项或页签容器。代码块中的围栏文字不参与容器匹配。支持在列表、引述、超级块和页签项中嵌套。

规范输出生成完整闭合标记。孤立的 `:::tab` 保留为文字；不完整输入按解析范围容错收束，不能丢失正文。结束标记后的 IAL 属于刚结束的节点，页签项与整组容器分别保存 ID 和属性。
