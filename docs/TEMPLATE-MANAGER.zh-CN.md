...
已有模板继续按原文件名读取，保留 Windows 换行及直接在文件末尾声明文档属性的旧写法。新导出的独立属性声明使用 `siyuan-template-doc-attrs-v1` 保存求值位置。移动和重命名不自动迁移外部模板引用，调用方需要同步调整引用路径。

文件版本冲突时保留当前编辑内容并拒绝覆盖。删除操作不生成文件历史，无法通过模板管理撤销；已有 `data/templates/.trash/` 内容继续保留且不自动清理。
