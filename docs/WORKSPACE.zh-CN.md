...
> 它与 [`SY-FORMAT.zh-CN.md`](./SY-FORMAT.zh-CN.md) 互补：后者说明 `.sy` 文件**内部**的 JSON 结构，而本文档说明工作区**整体**在文件系统中的布局。
> 所有结论基于真实工作区与内核代码核验。

## 0. 概述

SiYuan 工作区是一棵**自描述**的目录树：笔记本、文档和资源文件以文件或目录形式存在磁盘上。**文件名即 ID，目录结构即文档层级**。文件系统保存权威的原始内容，`temp/` 下的 SQLite 数据库则是可重建的索引和缓存。普通笔记本内容可以直接读取；而加密笔记本内容以密文落盘，必须通过已解锁的内核访问。
...
第三方主题位于 `data/themes/<name>/`，图标包位于 `data/icons/<name>/`。完整包内容、更新和卸载通过 `data/storage/appearance-v1/` 中不可变的内部归档参与数据同步及快照，本机主题和图标目录不直接进入普通文件同步。各设备在 `conf/conf.json` 中独立保存外观选择。内置主题和图标仍属于程序资源。即使资源文件设置为按需下载，外观包资源也会完整下载。旧客户端可以继续同步笔记，新客户端负责还原外观包及处理冲突。迁移会保留原有本地文件供恢复。迁移和冲突行为详见[外观同步](APPEARANCE-SYNC.zh-CN.md)。
...
- 展开子文档（`Ls`）：传入 `.sy` 路径，去掉 `.sy` 后缀，检查同名目录是否存在，若存在则列出其子项。
- 子文档深度（`GetChildDocDepth`）：同理 walk 同名目录。

> ⚠️ 这意味着：**移动或改名一个有子文档的文档，必须同步移动它的同名目录**，否则子文档将无法被定位。

### 顶层笔记本文档例外

启用顶层笔记本文档功能后，内核会在笔记本目录中直接创建 `<boxID>.sy`。该文档的 rootID 就是笔记本 ID，因此笔记本与其顶层文档共用同一个 ID。

开关保存在 `conf/conf.json` 的 `fileTree.boxDocEnabled` 中。新工作区默认启用；若已有配置中缺少该字段，则初始化为关闭。
...
`boxDoc.json` 包含元数据规范版本和顶层文档 ID。运行时内核直接从 `box.ID` 推导该文档 ID；而保留此元数据文件是为了让 `.sy.zip` 导入过程能够识别源笔记本的顶层文档，并将其重新映射为目标笔记本 ID。
...
加密笔记本必须使用笔记本本地资源文件。资源内容和原始名称存放在同一个加密容器中，物理文件名采用脱敏的 `<16 位随机字符>-<blockID>.<ext>` 格式，因此直接检查磁盘无法还原原始资源文件名称。详见 [加密文件格式](./ENCRYPTED-NOTEBOOK.zh-CN.md#sy--assets--数据库文件加解密)。
...
3. 把各级 title 用 `/` 连接，末尾拼接当前文档 title → 得到形如 `/父标题/子标题/当前标题` 的 HPath。
4. 若某父 `.sy` 缺失，内核会**自动创建**一个 `Untitled` 父文档（对应 issue #7376）。

> 这是「目录结构即数据」的根本原因：**移动或重命名文件/目录，会直接改变文档的 HPath 与面包屑**——因为 HPath 完全派生自目录链。

顶层笔记本文档会特殊处理：其 API HPath 为 `/`，完整的人类可读路径为笔记本名称；根层普通文档在逻辑上显示为它的下级文档，但文件仍然直接存放在笔记本目录中。

---

## 11. 直接操作文件系统的注意事项

> 与 [SY-FORMAT.zh-CN.md §0.5](./SY-FORMAT.zh-CN.md#05-何时直接读写-sy优先级) 一致：**修改数据优先通过 HTTP API / MCP / CLI**，由内核负责索引同步。直接操作文件系统仅用于批量离线迁移、冷初始化等场景。
...
| 文档 | 范畴 | 回答的问题 |
|---|---|---|
| **本文档（WORKSPACE）** | 文件系统层 | 「一个工作区在磁盘上如何组织？笔记本、文档和资源文件如何布局？」 |
| **SY-FORMAT** | AST 层 | 「一个 `.sy` 文件内部的 JSON 树是什么结构？有哪些节点类型和字段？」 |

两者互补：本文档说明 `.sy` 文件**放在哪、叫什么名、与谁同目录**；SY-FORMAT 说明 `.sy` 文件**里面写什么**。
