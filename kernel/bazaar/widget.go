// SiYuan - Refactor your thinking
// Copyright (c) 2020-present, b3log.org
//
// This program is free software: you can redistribute it and/or modify
// it under the terms of the GNU Affero General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// This program is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// GNU Affero General Public License for more details.
//
// You should have received a copy of the GNU Affero General Public License
// along with this program.  If not, see <https://www.gnu.org/licenses/>.

package bazaar

import (
	"path/filepath"

	"github.com/siyuan-note/logging"
	"github.com/siyuan-note/siyuan/kernel/util"
)

func InstalledWidgets() (ret []*Package) {
	ret = []*Package{}

	widgetsPath := filepath.Join(util.DataDir, "widgets")
	widgetDirs, err := readInstalledPackageDirs(widgetsPath)
	if err != nil {
		logging.LogWarnf("read widgets folder failed: %s", err)
		return
	}
	if len(widgetDirs) == 0 {
		return
	}

	bazaarWidgetsMap := buildBazaarPackagesMap("widgets", "")
	installedWidgetInfos := getInstalledPackageInfos(widgetDirs, "widget")

	for _, info := range installedWidgetInfos {
		widget := info.Pkg
		dirName := info.DirName

		config := PackageMetadataConfig{
			BasePath:          widgetsPath,
			DirName:           dirName,
			JSONFileName:      "widget.json",
			BaseURLPath:       "/widgets/" + dirName,
			BazaarPackagesMap: bazaarWidgetsMap,
		}

		if !setPackageMetadata(widget, config) {
			continue
		}

		ret = append(ret, widget)
	}
	return
}
