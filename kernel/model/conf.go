...
func InitConf() {
	...
	if util.AuthCLI.AccessAuthBypassSet {
		Conf.AccessAuthBypass = util.AuthCLI.AccessAuthBypass
	}
	...
	if util.AuthCLI.OIDCProviderSet {
		Conf.OIDC.Provider = util.AuthCLI.OIDCProvider
	}

	if util.AuthCLI.OIDCProvidersSet {
		if "" == util.AuthCLI.OIDCProviders {
			Conf.OIDC.Providers = map[string]*conf.OIDCProviderConf{}
		} else {
	...
	if util.AuthCLI.OIDCFiltersSet {
		if "" == util.AuthCLI.OIDCFilters {
			Conf.OIDC.Filters = map[string][]string{}
		} else {
