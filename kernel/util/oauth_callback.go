...
func RenderOAuthCallbackPage(lang, title, message string, success bool) []byte {
	return renderOAuthCallbackPage(lang, title, message, success, "")
}

// RenderOAuthRedirectPage 在同源结果页加载后进入已校验的站内目标，结束身份提供方的重定向链。
func RenderOAuthRedirectPage(lang, title, target string) []byte {
	return renderOAuthCallbackPage(lang, title, "", true, target)
}

func renderOAuthCallbackPage(lang, title, message string, success bool, target string) []byte {
	navigation, content := "", html.EscapeString(message)
	if target != "" {
		navigation = `<meta http-equiv="refresh" content="0;url=` + html.EscapeString(target) + `">`
		content = `<a href="` + html.EscapeString(target) + `">SiYuan</a>`
	}
	...
	return []byte(`<!doctype html><html lang="` + html.EscapeString(lang) + `"><head><meta charset="utf-8">` +
		`<meta name="viewport" content="width=device-width,initial-scale=1"><title>` + html.EscapeString(title) + `</title>` + navigation +
