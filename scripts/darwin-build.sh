...
if [[ "$TARGET" == 'amd64' || "$TARGET" == 'all' ]]; then
    ...
    export GOARCH=amd64
    go build -tags "fts5 sqlcipher" -o "../app/kernel-darwin/Jitang-Notes-Kernel" -ldflags "-s -w" .
fi
if [[ "$TARGET" == 'arm64' || "$TARGET" == 'all' ]]; then
    echo
    echo 'Building Kernel arm64'
    export GOARCH=arm64
    go build -tags "fts5 sqlcipher" -o "../app/kernel-darwin-arm64/Jitang-Notes-Kernel" -ldflags "-s -w" .
fi
