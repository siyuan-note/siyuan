...
if [[ "$TARGET" == 'amd64' || "$TARGET" == 'all' ]]; then
    ...
    export GOARCH=amd64
    setup_cc amd64
    go build -buildmode=pie -tags "fts5 sqlcipher" -o "../app/kernel-linux/Jitang-Notes-Kernel" -ldflags "-s -w -extldflags -static-pie" .
fi
if [[ "$TARGET" == 'arm64' || "$TARGET" == 'all' ]]; then
    ...
    export GOARCH=arm64
    setup_cc arm64
    go build -buildmode=pie -tags "fts5 sqlcipher" -o "../app/kernel-linux-arm64/Jitang-Notes-Kernel" -ldflags "-s -w -extldflags -static-pie" .
fi
