...
if defined BUILD_AMD64 (
    ...
    set GOARCH=amd64
    go build -tags fts5 -o "%PROJECT_ROOT%\app\kernel\SiYuan-Kernel.exe" -ldflags "-s -w -H=windowsgui" .
    if errorlevel 1 (
        exit /b %errorlevel%
    )
...
if defined BUILD_ARM64 (
    ...
    go build -tags fts5 -o "%PROJECT_ROOT%\app\kernel-arm64\SiYuan-Kernel.exe" -ldflags "-s -w -H=windowsgui" .
    if errorlevel 1 (
        exit /b %errorlevel%
    )
